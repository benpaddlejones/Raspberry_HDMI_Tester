...
- **ALSA target selection** – `detect-hdmi-audio` now returns the format-converting PCM aliases (`hdmiN_plug`) generated by `/etc/asound.conf`, falling back to `hdmi_auto` or `plughw:<card>,0` only when no alias is available. Forcing VLC through those wrappers keeps the plug layer active on vc4-hdmi devices and clears the "no supported sample format" failure on DRM-based boards.
- **PCM alias coverage** – `/etc/asound.conf` defines plug-enabled HDMI endpoints for cards 0–5, ensuring new firmware revisions or additional HDMI outputs remain format-compatible without touching the detection script.
...
- The `hdmiN_plug` wrappers guarantee format conversion even if `/etc/asound.conf` evolves or additional HDMI outputs appear, preventing failures when the hardware exposes S/PDIF-only formats.
